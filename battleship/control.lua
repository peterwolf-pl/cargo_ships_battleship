...
local function fire_at_enemy(entry)
  local ship = entry.ship
  if not (ship and ship.valid) then
    return
  end
  for _, turret in pairs(entry.turrets or {}) do
    if turret and turret.valid then
      local ammo_inventory = turret.get_inventory(defines.inventory.artillery_turret_ammo)
      if ammo_inventory and not ammo_inventory.is_empty() then
        local range = 40
        if turret.prototype and turret.prototype.attack_parameters and turret.prototype.attack_parameters.range then
          range = turret.prototype.attack_parameters.range
        end
        local target = turret.surface.find_nearest_enemy{
          position = turret.position,
          max_distance = range,
          force = ship.force
        }
        if target and target.valid and turret.shoot then
          turret.shoot{
            target = target,
            force = ship.force,
            position = turret.position
