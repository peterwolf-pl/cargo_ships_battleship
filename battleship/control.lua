...
local function ensure_globals()
  global.battleships = global.battleships or {}
end
...
local function ensure_entry(ship)
  ...
  ensure_globals()
  local entry = global.battleships[ship.unit_number]
  if not entry then
    entry = {ship = ship, turrets = {}}
    global.battleships[ship.unit_number] = entry
  else
    entry.ship = ship
  end
  sync_turrets(entry)
...
local function remove_ship(ship)
  ...
  if entry then
    destroy_entry(entry)
    global.battleships[ship.unit_number] = nil
  end
end

local function on_nth_tick()
  if not global.battleships then
    return
  end
  for unit_number, entry in pairs(global.battleships) do
    if not (entry.ship and entry.ship.valid) then
      destroy_entry(entry)
      global.battleships[unit_number] = nil
    else
      sync_turrets(entry)
      refill_ammo(entry)
    end
